...
def get_args_parser():
    ...
    parser.add_argument('--input-size', default=32,
    ...
    #add parameter
    parser.add_argument('--parallel-stages', type=int, nargs='+', default=None,
                    help='List of stage indices (e.g., 1) to use hybrid parallel attention.')
    parser.add_argument('--parallel-type', type=str, default='parallel',
                    help='Type of parallel attention structure: "parallel" (default) or "parallel_v2".')
...
def main(args):
    ...
    # 최대 GPU 메모리 사용량 계산 및 출력
    max_mem_gb = 0.0
    if torch.cuda.is_available():
        max_mem_gb = torch.cuda.max_memory_allocated() / (1024 ** 3)
        print(f'Peak GPU memory usage: {max_mem_gb:.2f} GB')
    print("Evaluating inference speed...")
    # 평가(eval) 모드로 설정하고, EMA(Exponential Moving Average) 가중치가 있다면 적용
    model_for_eval = model_without_ddp
    if args.model_ema:
        model_for_eval = model_ema.ema # EMA 모델 사용
    model_for_eval.eval()

    # (중요) 실제 속도 측정을 위해 데이터로더에서 딱 한 배치만 가져옴
    # data_loader_val는 배치 크기가 (1.5 * batch_size)이므로,
    # args.batch_size 크기의 더미 텐서로 측정하는 것이 더 표준적일 수 있음.
    # 여기서는 간단하게 val_loader의 첫 배치를 사용함.
    try:
        ...
        # GPU 예열(Warmup)
        for _ in range(10):
            _ = model_for_eval(samples)

        # 실제 측정
        torch.cuda.synchronize()
        start_time_inference = time.time()

        num_inference_batches = 50 # 50 배치 평균
        with torch.no_grad():
        ...
        actual_batch_size = samples.size(0) 
        images_per_second = (actual_batch_size * num_inference_batches) / total_inference_time
        print(f"Inference Speed: {images_per_second:.2f} images/sec")

    except Exception as e:
        print(f"Could not evaluate inference speed: {e}")
        images_per_second = 0.0



    if args.output_dir and utils.is_main_process():
        # 'log_stats' 변수가 루프의 마지막 상태를 가지고 있다고 가정합니다.
        # 만약 epoch=0에서 멈추면 log_stats가 정의되지 않을 수 있으니 확인이 필요합니다.
        # 안전하게 가려면 log_stats = {} 를 루프 시작 전에 선언해주는 것이 좋습니다.
        # (하지만 1 epoch는 돌기 때문에 괜찮을 것입니다.)
        final_summary = {
            'model': args.model,
            ...
            'last_epoch_stats': log_stats, # 루프의 마지막 log_stats
            'total_training_time_sec': total_time,
            'total_training_time_str': total_time_str,
            'peak_gpu_memory_gb': max_mem_gb,
            'n_parameters': n_parameters,
            'inference_images_per_sec': images_per_second
        }
        
        summary_path = output_dir / "final_summary.json"
        print(f"Saving final summary to {summary_path}")
        try:
