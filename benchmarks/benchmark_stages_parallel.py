...
def get_args():
    parser = argparse.ArgumentParser(description='Benchmark EfficientViT Stages (Parallel Stage 2)')
    ...
    parser.add_argument('--parallel-stages', type=int, nargs='+', default=None, help='stages to apply parallel attention')
...
def main():
    ...
    # Using a dummy loader just to ensure dataset is present, we use manual batch for timing
    
    # Load Model
    print(f"Loading model EfficientViT_M2_CIFAR with num_classes=10, parallel_stages={args.parallel_stages}...")
    ...
    # config['window_size'] = [4, 4, 2] # Removed override to match checkpoint
    
    # Initialize with parallel_stages from args
    ...
    with torch.no_grad():
        ...
        # 3. Stage 2 (blocks2) - This is the parallel one
